...
def maestra(request):
    if request.method == 'POST': # si existe un post
        ...
        if maestra_prod.is_valid():
            ...
            nuevo_producto.save()
            return render(request, 'eboxApp/windows.html') # una vez que se guardo que retorne a la pagina de confrimación, y desde la confirmacion window hice un html que me retorna a inicio
